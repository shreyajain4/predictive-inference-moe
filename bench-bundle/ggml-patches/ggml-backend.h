...
extern "C" {
    ...
    // ── moe-advisor: cache hook for copy_experts ─────────────────────────
    // When the scheduler is about to PCIe-transfer a run of MoE experts
    // [first_expert_id .. first_expert_id + n_experts_in_run) from a host
    // buffer (input) to a GPU compute buffer (input_cpy), it first calls
    // this hook. If the hook returns 1, the data was already written to
    // input_cpy_data + dst_offset by D→D copy from a GPU-side cache and
    // the scheduler skips the PCIe transfer. Return 0 to fall through to
    // the original H→D path.
    //
