...
// MoE copy_experts instrumentation. Always-on, near-zero per-call overhead.
// Dumps stats on process exit so we can measure how much PCIe traffic
// copy_experts is doing during a benchmark run.
namespace {
struct MoeCopyStats {
    ...
    ~MoeCopyStats() {
        ...
        fprintf(stderr,
                "\n[MOE-COPY-STATS] copy_experts calls=%llu  bytes=%.2f MiB  "
                "avg=%.1f KiB/call  PCIe-time-at-13GB/s=%.1f ms  at-200GB/s=%.1f ms\n",
                (unsigned long long)c,
                b / (1024.0 * 1024.0),
                (double)b / 1024.0 / (double)c,
                (b / 13.0e9) * 1000.0,
                (b / 200.0e9) * 1000.0);
    }
};
MoeCopyStats g_moe_copy_stats;
}

// moe-advisor expert cache hook (set by bench via ggml_set_moe_expert_cache_hook)
static ggml_moe_expert_cache_try_d2d_fn g_moe_expert_cache_hook = nullptr;
            ...
            if (input->flags & GGML_TENSOR_FLAG_INPUT) {
                ...
                if (split->graph.n_nodes > 0 &&
                    ...
                    auto copy_experts = [&](int32_t first_id, int32_t last_id) {
                        ...
                        const size_t total_bytes  = expert_size_copy + padding_end;

                        g_moe_copy_stats.calls.fetch_add(1, std::memory_order_relaxed);
                        g_moe_copy_stats.bytes.fetch_add(total_bytes, std::memory_order_relaxed);
